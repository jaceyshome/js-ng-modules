	...
	return {
		...
		template:
			'<div class="tinyCarouselSlider">' +
				'<a href="javascript:void(0)" onclick="return false;" class="buttonLeft" ' +
					'data-ng-class="{disable:carouselPanelX >= 0}" data-ng-click="onClickButtonBack($event)" ' +
					'title="previous group">' +
						'<span class="llsScreenReaderText">back</span>' +
				'</a>' +
			...
			'<div class="viewport">' +
				'<ul class="viewportList">' +
					'<li class="imagePanelContainer" data-ng-repeat="slide in slides">' +
						'<div class="imagePanel" data-ng-show="!isImageBtn && !slide.underOverlay">' +
							'<div class="carouselImageContainer">' +
								'<a class="imagePanelHoverButton" href="javascript:void(0)" data-slide-index="{{$index}}" ' +
									...
									'<img class="viewportImage" src="" data-ng-src="{{slide.imageLink}}" ' +
							...
							'<div class="overlayInner-block">' +
								'<div class="overlayInner">' +
									'<a class="buttonViewVideo" href="javascript:void(0)" onclick="return false;" ' +
										'title="This opens in a new window to view video of {{slide.title}}" ' +
										'data-ng-show="slide.videoLink" ' +
										'data-ng-click="launchVideo(slide.videoLink)">view video' +
									'</a>' +
									'<a class="buttonViewFile" href="" data-ng-href="{{slide.fileLink}}"  ' +
										'title="This opens in a new window to view pdf of {{slide.title}}" ' +
										'data-ng-show="slide.fileLink" target="_blank"' +
						...
						'<div class="imagePanel" data-ng-show="isImageBtn && !slide.underOverlay">' +
							'<a class="imagePanelBtn" href="javascript:void(0)" onclick="return false;" ' +
								'data-ng-click="onClickImageBtnPanel(slide)" title="select the badge {{$index}}">' +
								'<img class="viewportImage" src="" data-ng-src="{{slide.imageLink}}" ' +
				...
				'</div>' +
			'</div>' +
			'<a href="javascript:void(0)" onclick="return false;" data-ng-class="{disable:isLastGroup()}" ' +
				'class="buttonRight" data-ng-click="onClickButtonNext($event)" title="next group">' +
				'<span class="llsScreenReaderText">next</span>' +
			'</a>' +
		'</div>',
		...
		link:function ($scope, $element, $attrs){
			...
			// ---------------------------------------------------------------------------- $scope Functions
			$scope.launchVideo = function launchVideo () {
			...
			$scope.onClickButtonBack = function(event){
				event.target.blur();
				if($scope.isOnAnimation || $scope.carouselPanelX >= 0){
					return;
				}
			...
			$scope.onClickButtonNext = function(event){
				event.target.blur();
				if($scope.isOnAnimation || $scope.isLastGroup()){
					return;
				}
				showAllSlides(function(){
					(animateSlideList.call(null, ($scope.carouselPanelX -= xOffset) ));
