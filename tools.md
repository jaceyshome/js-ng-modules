# The Tools Used in `myApp`

## Introduction

`myApp` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `myApp` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`myApp` uses git to manage its codebase. While in theory you don't have
to use Git once you download `myApp`, this project makes the assumption
...
While `myApp` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `myApp` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `myApp`; Grunt is
...
to get started with `myApp`. But for those curious or looking to go a
...
Grunt is the engine behind `myApp`. It's the magic that makes it move.
...
`myApp` comes with a `bower.json` file that looks something like this:
...
in `myApp` by default in there to see what I mean.
